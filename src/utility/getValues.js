...
getValues.generateAccessToken = async (user) => {
  return jwt.sign(user, config.jwt.accesstoken, { expiresIn: `${config.jwt.accesstokenExpiry}d` });
};

getValues.generateRefreshToken = async (user) => {
  return jwt.sign(user, config.jwt.refreshtoken, { expiresIn: `${config.jwt.refreshtokenExpiry}d` });
};

getValues.decodeAccessToken = (token) => {
  const decoded = jwt.verify(token, config.jwt.accesstoken);
  return decoded;
};

getValues.decodeRefreshToken = (token) => {
  const decoded = jwt.verify(token, config.jwt.refreshtoken);
  return decoded;
};
...
getValues.generatePassword = () => {
  ...
  const password = jwt.sign(payload, config.jwt.accesstoken, { header });
  return password;
};
...
getValues.updatePassword = (password) => {
  ...
  return jwt.sign(payload, config.jwt.accesstoken, { header });
};
